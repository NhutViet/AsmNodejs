    ...
    if (token) {
      JWT.verify(token, config.SECRETKEY, async function (err, id) {
        if (err) {
          ...
          res
            .status(200)
            .json({ status: true, message: "Thêm Book thành công" });
        }
      });
    } else {
      res.status(401).json({ status: false, message: "không xác thực" });
      ...
      JWT.verify(token, config.SECRETKEY, async function (err, id) {
        if (err) {
          ...
          if (findBook) {
            ...
            res
              .status(404)
              .json({ status: false, message: "Không tìm thấy ID" });
          }
        }
      });
    } else {
      ...
      JWT.verify(token, config.SECRETKEY, async function (err, id) {
        if (err) {
          ...
          if (findBook) {
            ...
            res
              .status(404)
              .json({ status: true, message: "Không tìm thấy ID" });
          }
        }
      });
    } else {
